"""Metric domain entity for the Semantic component.
...
Links to Concept from the Identity component via concept_id.
"""
...
class Metric:
    ...
    semantic identity and cross-dataset alignment.
    ...
    def _validate_invariants(self) -> None:
        ...
        if self.kind != expected_kind:
            raise ValueError(
                f"kind {self.kind.value} does not match spec type {type(self.spec).__name__}, "
                f"expected kind {expected_kind.value}"
            )
