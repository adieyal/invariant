...
Identity **assesses** comparability factors but does not decide query outcomes:

| Identity Provides | Consumer |
|-------------------|----------|
| Concept match (same/different) | Validation |
| Universe alignment (full/partial/disjoint) | Validation |
| Unit compatibility (convertible/incompatible) | Validation |
| Cached comparability assertions | Validation |

Validation calls `Qry:AssessCompatibility` and combines the result with Reference crosswalk data to produce final Issues.
