...
Semantic is a **suite** of related capabilities. Keep separate application services to prevent monolith creep:

| Sub-domain | Responsibility | Application Service |
|------------|----------------|---------------------|
| **Metrics** | Metric definitions, DAG, topological sort | `MetricService` |
| **Dimensions** | Dimension definitions, attribute expressions | `DimensionService` |
| **SemanticDataset** | Logical-to-physical binding, grain mapping | `DatasetBindingService` |
| **GeoHierarchy** | Geographic structure, rollup rules | `GeoHierarchyService` |
| **Materialization** | Pre-computed view registry, refresh policy | `MaterializationService` |

**Rules:**
- One repo/module is acceptable, but services must not cross sub-boundaries
- `MetricService` must not know about materialization internals
- `GeoHierarchyService` must not know about metric DAG logic
- Cross-cutting queries (e.g., `GetSemanticCatalog`) aggregate from sub-services
...
| Query | Description |
|-------|-------------|
| `Qry:ResolveMetric(name) -> MetricDTO | None` | Resolve metric by name (returns None if not found) |
| `Qry:GetMetricById(metric_id) -> MetricDTO` | Get metric by ID (raises MetricNotFoundError) |
| `Qry:ResolveDimension(name) -> DimensionDTO | None` | Resolve dimension by name (returns None if not found) |
| `Qry:GetDimensionById(dimension_id) -> DimensionDTO` | Get dimension by ID (raises DimensionNotFoundError) |
...
- **Resolution by name** returns `None` if not found (e.g., `ResolveMetric("unknown")` → `None`)
- **Get by ID** raises error if not found (e.g., `GetMetricById(MetricId)` → `Err:MetricNotFoundError`)
